...
function CustomerBikes() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100 flex flex-col">
      <CustomerNavBar />
      {/* Hero Section */}
      <section className="relative bg-gradient-to-r from-blue-600 to-blue-400 text-white py-16 shadow-md mb-10">
        <div className="max-w-5xl mx-auto px-6 flex flex-col items-center text-center">
          <h1 className="text-5xl font-extrabold tracking-tight mb-4 drop-shadow-lg">Discover Your Dream Ride</h1>
          <p className="text-lg font-medium mb-6 max-w-2xl">Explore our latest collection of brand new bikes, handpicked for performance, style, and value. Find the perfect match for your journey!</p>
        </div>
      ...
      <main className="flex-grow px-4 pb-10">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-3xl font-bold text-blue-900 mb-8 text-center tracking-wide underline decoration-blue-900 decoration-2 underline-offset-8">Brand New Bikes</h2>
          {bikes.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {bikes.map((bike) => (
                <div
                  key={bike._id}
                  className="bg-white/90 rounded-xl shadow-xl hover:shadow-2xl transition-all hover:scale-[1.03] p-0 border border-blue-100 flex flex-col"
                >
                  <div className="mb-4 relative">
                    ...
                    <div
                      className={`w-full h-56 bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center rounded-t-xl border-b border-blue-100 ${
                        bike.image ? "hidden" : ""
                      }`}
                    >
                  ...
                  <div className="bg-gray-100 px-6 py-3 rounded-b-l flex flex-col items-start mb-2 border-b border-gray-200">
                    <h3 className="text-xl font-bold text-gray-800 mb-1">{bike.model}</h3>
                    <div className="text-sm text-gray-400 font-semibold">Type: {bike.type}</div>
                  </div>
